...
do
	...
	do
		...
		#for (( inodes = 97; inodes <= blocks/2; inodes++ ))
		#do
			echo "Blocks: $blocks"
			echo "inodes: $inodes"
			...
			prof_output=$(../bin/mksofs_bin -c $clusters -i $inodes $name)
			content_prof=$(../bin/showblock -s 0 $name)
			echo "" > "prof_Outp"
			...
			group_output=$(../bin/mksofs -c $clusters -i $inodes $name)
			content_group=$(../bin/showblock -s 0 $name)
			echo "" > "group_Outp"
